...
use otter_identity::{trust::TrustStore, PublicIdentity};
...
const ENCRYPTED_IDENTITY_MAGIC: &str = "otter-identity-v1";
...
impl FileStorage {
    ...
    fn derive_identity_key(passphrase: &str) -> [u8; 32] {
        blake3::derive_key("otter-identity-encryption-v1", passphrase.as_bytes())
    }

    pub fn encrypt_identity_json_bytes(
        identity_json: &str,
        passphrase: &str,
    ) -> Result<Vec<u8>, StorageError> {
        let key = Self::derive_identity_key(passphrase);
        ...
        let wrapped = EncryptedIdentityFile {
            magic: ENCRYPTED_IDENTITY_MAGIC.to_string(),
    ...
    pub fn decrypt_identity_json_bytes(
        data: &[u8],
        passphrase: &str,
    ) -> Result<String, StorageError> {
        let wrapped: EncryptedIdentityFile = serde_json::from_slice(data)
            .map_err(|e| StorageError::DeserializationError(e.to_string()))?;

        if wrapped.magic != ENCRYPTED_IDENTITY_MAGIC {
            return Err(StorageError::InvalidData(
                "Unsupported identity file format".to_string(),
            ));
        }
        ...
        let nonce_array: [u8; 12] = nonce_bytes
            .try_into()
            .map_err(|_| StorageError::InvalidData("Invalid nonce length".to_string()))?;
        ...
        String::from_utf8(plaintext).map_err(|e| StorageError::DeserializationError(e.to_string()))
    }
    ...
    pub async fn save_identity_encrypted(
        ...
        let data = Self::encrypt_identity_json_bytes(identity_json, passphrase)?;
    ...
    pub async fn load_identity_encrypted(
        ...
        Ok(Some(Self::decrypt_identity_json_bytes(&data, passphrase)?))
    }
}
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match self.read_file(&path).await {
                    Ok(data) => match serde_json::from_slice::<SessionData>(&data) {
                        Ok(session) => {
...
mod tests {
    ...
    fn test_identity_encryption_roundtrip() {
        ...
        let encrypted = FileStorage::encrypt_identity_json_bytes(json, passphrase).unwrap();
        let decrypted = FileStorage::decrypt_identity_json_bytes(&encrypted, passphrase).unwrap();

        assert_eq!(decrypted, json);
    }

    #[test]
    fn test_identity_encryption_wrong_passphrase_fails() {
        let encrypted = FileStorage::encrypt_identity_json_bytes("{}", "good-pass").unwrap();
        let result = FileStorage::decrypt_identity_json_bytes(&encrypted, "bad-pass");
