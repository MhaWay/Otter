...
impl CryptoSession {
    ...
    pub fn new(
        ...
        let shared_secret = local_identity
            .encryption_secret_key()
            .diffie_hellman(&remote_key);
...
impl PFSSession {
    ...
    pub fn new(
        ...
        let remote_key = remote_public.encryption_public_key()?;
        let static_secret = local_identity
            .encryption_secret_key()
            .diffie_hellman(&remote_key);
    ...
    /// Create a PFS session from a precomputed ephemeral shared secret.
...
impl MessageCrypto {
    ...
    pub fn serialize(encrypted: &EncryptedMessage) -> Result<String, CryptoError> {
        serde_json::to_string(encrypted).map_err(|e| CryptoError::SerializationError(e.to_string()))
    }
